...
public class BlobDiffPage extends RepositoryPage {
	...
	public BlobDiffPage(PageParameters params) {
		...
		if (StringUtils.isEmpty(baseObjectId)) {
			...
			diff = DiffUtils.getDiff(r, commit, blobPath, diffComparator, DiffOutputType.HTML_UNIFIED, handler, 3).content;
			...
			diff = DiffUtils.getDiff(r, baseCommit, commit, blobPath, diffComparator, DiffOutputType.HTML_UNIFIED, handler,
									 3).content;
			if (handler.getImgDiffCount() > 0) {
				addBottomScript("scripts/imgdiff.js"); // Tiny support script for image diffs
			}
